...
namespace FiveMinute.Controllers {
	public class AccountController(
		...
		public async Task<IActionResult> Register(RegisterViewModel registerViewModel) {
			...
			if (newUserResponse.Errors.First().Description.Contains("Password")) {
		...
		public async Task<IActionResult> All() {
			...
			var allUsersViewModel = new AllUsersViewModel {
				Admins = users.Where(u => u.UserRole == UserRoles.Admin)
				              .Select(u => new UserIdNameEmailViewModel(u.Id, u.UserName, u.Email)).ToList(),
				Teachers = users.Where(u => u.UserRole == UserRoles.Teacher)
				                .Select(u => new UserIdNameEmailViewModel(u.Id, u.UserName, u.Email)).ToList(),
				Students = users.Where(u => u.UserRole == UserRoles.Student)
				                .Select(u => new UserIdNameEmailViewModel(u.Id, u.UserName, u.Email)).ToList(),
			};
